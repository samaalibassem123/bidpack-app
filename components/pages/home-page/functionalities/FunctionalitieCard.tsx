...
export default function FunctionalitieCard({ children, className }: Props) {
  return (
    <SlideUpScrollAnimation delay={0.5}>
      <div
        className={cn(
          "relative w-full cursor-pointer drop hover:scale-101 hover:opacity-95 group scale-100 hover:shadow-gray-400/10 shadow-lg transition-all duration-200 flex flex-col justify-end gap-3 bg-[#FFFFFF1A] sm:w-[280px] text-start    p-[20px] pl-[43px]  h-[280px]  rounded-[10px] border border-white/20",
          className
        )}
      >
        {children}
